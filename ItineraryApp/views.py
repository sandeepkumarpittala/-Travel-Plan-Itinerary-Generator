...
def UserLoginAction(request):
    if request.method == 'POST':
        ...
        index = 0
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'itinerary',charset='utf8')
        with con:    
            cur = con.cursor()
        ...
        with con:    
            ...
            for row in rows:
                if row[0] == username:
                    ...
                    break
        if status == "none":
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'itinerary',charset='utf8')
